...
export default class ThpaceGL extends ThpaceBase {
	...
	generateVertices() {
		...
		for (let i = 0; i < points.length; i += 2) {
			noisedPoints.push([
				points[i] + noiseValues[i] * noiseSetting,
				points[i + 1] + noiseValues[i + 1] * noiseSetting,
			]);
		}
		...
		for (let i = 0; i < screenSpace; i++) {
			...
			for (let j = 0; j < toJ; j++) {
				...
				if (Array.isArray(variationX))
					variationX = getRandomNumber(variationX[0], variationX[1]);
				newData.variationX.push(variationX);

				let variationY = settings.variationY!;
				if (Array.isArray(variationY))
					variationY = getRandomNumber(variationY[0], variationY[1]);
	...
	init() {
		...
		const uPointVariationX = gl.getUniformLocation(
			this.triangleShaderProgram,
			'uPointVariationX',
		);
		gl.uniform1f(uPointVariationX, this.settings.pointVariationX!);
		const uPointVariationY = gl.getUniformLocation(
			this.triangleShaderProgram,
			'uPointVariationY',
		);
		...
		const uPointAnimationSpeed = gl.getUniformLocation(
			this.triangleShaderProgram,
		...
		// animation offset
		const uAnimationOffset = gl.getUniformLocation(
			this.triangleShaderProgram,
			'uAnimationOffset',
		);
